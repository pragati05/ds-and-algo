...
public class BinarySearchRecursiveOnRotateArray2 {
	...
	static int binarySearch(int[] input,int left,int right,int searchElement) {
		...
		if(right < left) {
			...
			if(searchElement >= input[left] && searchElement < input[mid]) {
				right = mid-1;
			}else {
				left = mid+1;
			}
			...
			if(searchElement > input[mid] && searchElement <=input[right]) {
				left = mid+1;;
			}else {
				right = mid-1;
			}
